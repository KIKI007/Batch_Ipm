...
class IpmSim(torch.nn.Module):
    ...
    def GTZSG_(self, p, ZS, nλn, nt, nf, mu):
        nbatch = p.shape[1]
        nλt = nλn * nt
        nx = nλn + nλt + nf
    ...
    def Q_(self, p, nλn, nt, iA, iB, nA, nB, invM):
        batch = p.shape[1]
        nλt = nλn * nt
    ...
    def index_mapping(self, batch_part_states, iAs, iBs, nλn, device):
    ...
    def ipm_get_dynamic_attrib(self, ipm, batch_part_states):
        ...
        ps, cs = self.index_mapping(batch_part_states, ipm.iAs, ipm.iBs, ipm.nλn, device=device)
    ...
    def sum_forces(self, p, nλn, nt, iA, iB, nA, nB):
        batch = p.shape[1]
        nλt = nλn * nt
    ...
    def ipm_evaluate_result(self, ipm, xclip, ps):
        batch = xclip.shape[1]
        rbeQ = ipm.nλn, ipm.nt, ipm.iAs, ipm.iBs, ipm.nAs, ipm.nBs
        residual = (self.sum_forces(xclip, *rbeQ) + ipm.g[:, None]) * ps
    ...
    def ipm_kkt_res(self, ipm, q, h, x, s, z):
        rbe = ipm.nλn, ipm.nt, ipm.nf, ipm.mu
        rbeQ = ipm.nλn, ipm.nt, ipm.iAs, ipm.iBs, ipm.nAs, ipm.nBs, ipm.invM
        r1 = self.Q_(x, *rbeQ) + q + self.GT_(z, *rbe)
        r2 = s * z
        r3 = self.G_(x, *rbe) + s - h
        kkt_res = inf_norm(torch.vstack([r1, r2, r3]))
        return r1, r2, r3, kkt_res

    def ipm_precond(self, ipm, s, z):
        ZS = z / s
    ...
    def centering_params(self, s, z, ds_a, dz_a, n_sample):
        """duality gap + cc term in predictor-corrector PDIP"""
        sz = torch.sum(s * z, dim=0)
        mu = sz / s.shape[0]
        alpha = self.ipm_linesearch(s, ds_a, z, dz_a, n_sample=n_sample)
        sigma = torch.sum((s + alpha * ds_a) * (z + alpha * dz_a), dim=0) / sz
        sigma = sigma ** 3
        return sigma, mu

    def ipm_solve_rhs(self, ipm, s, z, invP, v1, v2, v3, n_iter, dx=None):
        device = ipm.device
        ...
        if dx is None:
            ...
            xk = dx.clone()
            rk = b - (self.GTZSG_(dx, ZS, *rbeG) + self.Q_(dx, *rbeQ))
            dx = torch.zeros_like(b)
        ...
        # self.reset_timer('pcg')
        for k in range(m):
            for t in range(ipm.n_pcg_eval_iter):
                # Apk = GT @ (ZS * (G @ pk)) + Q @ pk
                # fast computation
                Apk = self.GTZSG_(pk, ZS, *rbeG) + self.Q_(pk, *rbeQ)
            ...
            rk = b - (self.GTZSG_(xk, ZS, *rbeG) + self.Q_(xk, *rbeQ))
            uk = invP * rk

            if rel < ipm.pcg_rel_eps:
                break

        ds = v3 - self.G_(dx, *rbeG)
        dz = (v2 - z * ds) / s
        return dx, ds, dz

    def get_dynamic_attrib(self, batch_part_states, rbe, float_type):
        ps, cs = rbe.mapping(batch_part_states)
        q, xl, xu, Al, Au = compute_rbe_dynamic_attribs(rbe.g, ps, cs, rbe.Jn, rbe.Jt, rbe.invM, rbe.mu, rbe.Ccp)
    ...
    def simulate_ipm(self, batch_part_states: list[dict], ipm_settings):
        ...
        q, xl, xu, Al, ps, cs = self.ipm_get_dynamic_attrib(ipm, batch_part_states)
        xl, xu = xl - ipm.x_bound_tol, xu + ipm.x_bound_tol
        h = torch.vstack([-Al, -xl, xu])

        # initialize ipm x0
        x, s, z = self.ipm_start_solve(ipm, h, q)
        ...
        r1, r2, r3, kkt_res = self.ipm_kkt_res(ipm, q, h, x, s, z)

        # ipm main loop
        for it in range(ipm.n_iter):

            # 1. pre-conditioner
            invP = self.ipm_precond(ipm, s, z)

            # 2. solve kkt 1
            dx_a, ds_a, dz_a = self.ipm_solve_rhs(ipm, s, z, invP, -r1, -r2, -r3, n_iter=ipm.n_pcg_iter)

            # 3. centering parameters
            sigma, mu = self.centering_params(s, z, ds_a, dz_a, n_sample=ipm.n_linesearch)
            ...
            # option 2
            r2 = (sigma * mu - (ds_a * dz_a))
            dx, ds, dz = self.ipm_solve_rhs(ipm, s, z, invP, 0, r2, 0, n_iter=ipm.n_pcg_iter)
            dx, ds, dz = dx + dx_a, ds + ds_a, dz + dz_a

            # 5. step size and update
            alpha = 0.99 * self.ipm_linesearch(s, ds, z, dz, n_sample=ipm.n_linesearch)
